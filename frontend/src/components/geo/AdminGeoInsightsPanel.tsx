...
import { AdminActivityMap, activityMapPointKey } from "./AdminActivityMap";
...
export function AdminGeoInsightsPanel() {
  ...
  const hiddenCount = Math.max(0, sortedPoints.length - TOP_CITIES_COUNT);
  const listPoints = legendExpanded
    ? sortedPoints
    : sortedPoints.slice(0, TOP_CITIES_COUNT);

  const handleCitySelect = (point: ActivityMapPoint) => {
      ...
      <div className="admin-geo-grid">
        <div className="admin-geo-map-card">
          ...
          {loading ? (
            ...
            <>
              <AdminActivityMap data={mapData} focusPointKey={selectedKey} />

              {sortedPoints.length > 0 && (
                <div className="admin-geo-legend">
                  <p className="admin-geo-legend-title">Ciudades con más actividad</p>
                  <ul
                    className={`admin-geo-legend-list${
                      legendExpanded ? " admin-geo-legend-list--expanded" : ""
                    }`}
                  >
                    {listPoints.map((p) => {
                      const key = pointKey(p);
                      const rank =
                        sortedPoints.findIndex((row) => pointKey(row) === key) + 1;
                      const pct =
                        mapData.total_events > 0
                      ...
                      return (
                        <li key={`${key}-legend`}>
                          ...
                          >
                            <span className="admin-geo-legend-rank">{rank}</span>
                            <span
                  ...
                  {hiddenCount > 0 && (
                    <button
                      type="button"
                      className="admin-geo-legend-toggle"
                      aria-expanded={legendExpanded}
                      onClick={() => setLegendExpanded((open) => !open)}
                    >
                      <PrimeIcon
                        name={legendExpanded ? "pi-chevron-up" : "pi-chevron-down"}
                        size={13}
                      />
                      {legendExpanded
                        ? "Ver solo top 5"
                        : `Ver ${hiddenCount} ciudad${hiddenCount === 1 ? "" : "es"} más`}
                    </button>
                  )}
                </div>
              )}
