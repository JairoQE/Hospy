...
export function ratingLabel(score: number, language?: Language): string {
  ...
  if (score >= 8) return translate("detail.rating.veryGood", lang);
  if (score >= 7) return translate("detail.rating.good", lang);
  if (score >= 6) return translate("detail.rating.pleasant", lang);
